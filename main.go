...
func (s *signature) minDarts() int {
	...
	if x := 2 * s.deg1Vertices(); x > cnt {
		cnt = x
	}
	return cnt
}

// deg1Vertices returns the number of leaves in this map.
func (s *signature) deg1Vertices() (cnt int) {
	for _, d := range s.degs {
		if d == 1 {
			cnt++
		}
...
func div(a, b int64) int64 {
	if a < 0 {
		panic(fmt.Sprintf("overflow suspected: %d", a))
	}
	if a%b != 0 {
		panic(fmt.Sprintf("not divisible: %d/%d", a, b))
	}
	return a / b
}

// orientableMaps returns the number of maps on orientable orbifolds.
func orientableMaps(g, h, n, innervf, dangling int) (cnt int64) {
	for d := 0; d <= n; d++ {
		fixedInnerVF := innervf
		if fixedInnerVF != IRRELEVANT {
			fixedInnerVF += dangling
		}
		s := signature{
			genus:   g,
			darts:   n + dangling,
			deg0:    d,
			innervf: fixedInnerVF,
		}
		for i := 0; i < h; i++ {
			s.holes = append(s.holes, hole{})
		}
		for i := 0; i < dangling; i++ {
			s.degs = append(s.degs, 1)
		}
		cnt += div(div(s.count(), fact(h)), fact(dangling))
		...
		cnt += int64(d) * div(div(s.count(), fact(h-1)), fact(dangling))
		...
		cnt += int64(2*d+1) * div(div(s.count(), fact(h-1)), fact(dangling))
		...
		cnt += int64(d+1) * div(div(s.count(), fact(h-1)), fact(dangling))
	}
	return cnt
}

// orientableMaps returns the number of maps on non-orientable orbifolds.
func nonOrientableMaps(cc, h, n, innervf, dangling int) (cnt int64) {
	for d := 0; d <= n; d++ {
		fixedInnerVF := innervf
		if fixedInnerVF != IRRELEVANT {
			fixedInnerVF += dangling
		...
		// h+dangling because the code assumes the chosen point has a root which
		// has come from splitting a vertex and may have 2x more.
		cnt += div(div(div(s.count(), fact(h)), pow(2, h+dangling)), fact(dangling))
		...
		cnt += int64(d) * div(div(div(s.count(), fact(h-1)), pow(2, h-1+dangling)), fact(dangling))
		...
		cnt += int64(2*d+1) * div(div(div(s.count(), fact(h-1)), pow(2, h-1+dangling)), fact(dangling))
		...
		cnt += int64(d+1) * div(div(div(s.count(), fact(h-1)), pow(2, h-1+dangling)), fact(dangling))
	}
	if cc%2 == 0 {
		cnt -= orientableMaps(cc/2, h, n, innervf, dangling)
	}
	return cnt
}
...
func countOnOrbifold(darts int, orientable bool, chi, h int, bp ...int) (cnt int64) {
	...
	for i := 0; i <= m2; i++ {
		...
		for vf := 0; vf <= darts+1; vf++ {
			...
			if (h == 0 && darts%2 != i%2) || i > darts {
				continue
			}
			if orientable {
				cnt += orientableMaps((2-chi-h)/2, h, darts, vf, i) * distrbp(vf, m)
			} else {
				cnt += nonOrientableMaps(2-chi-h, h, darts, vf, i) * distrbp(vf, m)
			}
			//fmt.Println(i, vf, h, cnt, darts, distrbp(vf, m))
			if vf == IRRELEVANT {
				break
			}
		}
...
func main() {
	...
	for _, ln := range strings.Split(string(data), "\n") {
		...
		var Chi, L, chi, H int
		var signStr string
		if _, err := fmt.Sscanf(ln, "%d%d%s%d%d ", &Chi, &L, &signStr, &chi, &H); err != nil {
			fmt.Println(err)
			os.Exit(1)
		}
		CC := 2 - Chi
		...
		var Epi, EpiPlus int64
		if _, err := fmt.Sscanf(splits[1], "%d%d", &Epi, &EpiPlus); err != nil {
